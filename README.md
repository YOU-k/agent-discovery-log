...
`scripts/publish_xhs.py` picks ONE project worth talking about
(highest-score novel find of the week, `--repo owner/name` to override),
gathers real material — full README, tracking data, HN discussions, and
optional Tavily web context (`TAVILY_API_KEY`) — and writes a deep-dive
note: what it is, why it's hot, how to actually use it, who it's for.
Facts come only from the gathered material; the body gets the
de-AI-slop pass. Writes `publish/YYYY-MM-DD-xhs.md`, records coverage in
`publish/.covered.json` (no repeats), pushes a red Feishu card. Runs in
the daily workflow, no-op except Mondays; `--force` to generate any day.
小红书 has no official personal publishing API — posting is manual by
design (cookie-based RPA exists but risks the account).
